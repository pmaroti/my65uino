...
wait_long:
    ...
    sta cntr1
    lda DRB
    and #0b0100_0000  ; mask input button state on bit 6
    beq skip_decrement ; if button not pressed, skip decrement
    lda #10
    sta cntr1          ; if button pressed, set shorter wait
skip_decrement:
wait1:
    lda #0xFF
    sta WTD64DI         ; start timer with max value, divide by 64, disable interrupt
wait_timer:
    ;lda READTDI         ; read timer value, disable interrupt
    ;bne wait_timer
    lda RRIFR
    bpl wait_timer   ; wait for timer flag set, if positive (bit 7 = 0), keep waiting
    dec cntr1
    bne wait1
    rts
